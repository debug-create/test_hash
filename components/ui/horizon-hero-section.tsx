...
// HeroSection.jsx reference component as requested
...
export const Component = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[300vh] bg-black text-white">
      ...
      <div ref={menuRef} className="fixed top-8 left-8 z-20 flex items-center gap-3 text-xs tracking-widest uppercase text-zinc-400 font-mono">
        <div className="flex flex-col gap-1 w-4">
          <span className="h-0.5 w-full bg-zinc-300"></span>
          <span className="h-0.5 w-2/3 bg-zinc-300"></span>
          <span className="h-0.5 w-full bg-zinc-300"></span>
        </div>
        <span>SPACE</span>
      </div>
      ...
      <div className="fixed inset-0 z-10 flex flex-col items-center justify-center pointer-events-none">
        <h1 ref={titleRef} className="text-7xl md:text-9xl font-black tracking-tighter text-[#ff3333] drop-shadow-[0_0_40px_rgba(255,51,51,0.5)]">
          {'HORIZON'.split('').map((char, i) => (
            <span key={i} className="title-char inline-block">{char}</span>
          ))}
        </h1>
        
        <div ref={subtitleRef} className="mt-6 text-center text-zinc-300 text-sm md:text-base font-light tracking-wide max-w-md">
          <p className="subtitle-line">Where vision meets reality,</p>
          <p className="subtitle-line">we shape the future of tomorrow</p>
        </div>
      </div>

      <div ref={scrollProgressRef} className="fixed bottom-8 right-8 z-20 flex items-center gap-3 text-xs font-mono text-zinc-400">
        <span>SCROLL</span>
        ...
        <span>{String(currentSection).padStart(2, '0')} / {String(totalSections).padStart(2, '0')}</span>
      </div>
    </div>
  );
};
